...
3. 站点同步：`npm.cmd run ops:update`
4. 产物生成：更新 `site/data/resources.json`、`site/data/feed.json`、`site/sitemap.xml`、`docs/operations/daily-ops-report.md`、`docs/operations/market-research.md`。
5. 人工审核：把高质量 candidate 改为 `active` 后再公开展示。
6. 日报汇总：由 `auto-creater/scripts/daily_profit_summary.py` 汇总进本地盈利项目日报。
...
- 第二阶段：把高频内容打包成模板、清单、提示词和小工具。
- 第三阶段：加入联盟分销、资料包下载、咨询入口。
